...
function buildPracticeArena() {
  addPlatform(0, -30, 220, 220, 0, 0x8e8e8e);
  addPlatform(0, 16, 12, 16, 0, 0x9f9f9f);
  ...
  addPlatform(12, -28, 8, 8, 3.9, 0xa5a5a5);
  addTower(0, -44, 5.5);
  addTower(6, -44, 5.5);
  addBar(new THREE.Vector3(3, 4.5, -44));
  addWall(-18, -54, 7, 1.2, 0, 5.4, 0x767676, true);
  addClimbHolds();
  addPlatform(-18, -60, 9, 9, 4.1, 0xa7a7a7);
  addPlatform(22, -18, 6, 6, 1.4, 0xa0a0a0);
  addRail(new THREE.Vector3(22, 2.2, -20), new THREE.Vector3(22, 1.2, -38));
  addJumpPad(-26, -20);
  addPlatform(-26, -34, 10, 10, 6.2, 0xafafaf);
  addFan(30, -24);
  addPlatform(30, -36, 10, 10, 3.8, 0xacacac);
}

function addPlatform(x, z, width, depth, topY, color) {
  ...
  world.surfaces.push({ minX: x - width / 2, maxX: x + width / 2, minZ: z - depth / 2, maxZ: z + depth / 2, y: topY });
}
...
function addBar(center) {
  ...
  mesh.castShadow = true;
  world.group.add(mesh);
  world.grabbables.push({ type: "bar", center });
}
...
function addClimbHolds() {
  ...
  for (let row = 0; row < ys.length; row += 1) {
    for (let col = 0; col < xs.length; col += 1) {
      ...
      mesh.castShadow = true;
      world.group.add(mesh);
      world.grabbables.push({ type: "hold", point, clusterId: "practice-climb" });
    }
  }
}
...
function onPointerLockChange() {
  ...
  if (!locked) prompt.textContent = "Click in the arena to re-enter the run.";
}
...
function attemptAirDash() {
  ...
  player.velocity.copy(getCameraForward().multiplyScalar(AIR_DASH_SPEED));
  return true;
}
...
function resolveGround(previousY) {
  ...
  for (const surface of world.surfaces) {
    ...
    if (previousY >= surface.y - 0.2 && player.position.y <= surface.y + 0.25 && (!bestSurface || surface.y > bestSurface.y)) bestSurface = surface;
  }
...
function updateWallContact() {
  ...
  for (const collider of world.colliders) {
    ...
    if ((keys.get("ShiftLeft") || keys.get("ShiftRight")) && lookForward.dot(normal) < -0.12 && player.velocity.y < 0) player.wallSliding = true;
    return;
  }
}
...
function tryStartGrab() {
  ...
  if (!best) return;
  if (best.type === "bar") player.grab = { type: "bar", target: best, swingAngle: 0, swingVelocity: THREE.MathUtils.clamp(player.velocity.z * -0.05, -1.2, 1.2) };
  else {
    player.grab = { type: "hold", target: best };
    player.position.copy(best.point).add(new THREE.Vector3(0, -1.18, 0));
  }
  player.velocity.set(0, 0, 0);
  player.grounded = false;
  player.airDashReady = true;
...
function updateGrab(delta) {
  ...
  player.position.copy(hold.point).add(new THREE.Vector3(0, -1.18, 0));
...
function releaseGrab(fromMouseUp) {
  if (!player.grab) return;
  if (player.grab.type === "bar") {
  ...
  } else if (fromMouseUp) player.velocity.y = Math.max(player.velocity.y, 1.4);
  player.grab = null;
}
